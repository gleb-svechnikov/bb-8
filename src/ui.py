...
class Button:
    ...
    def draw(self, surface):
        fill = self.fill if self.enabled else GREY
        ...
        if self.icon:
            self.icon(surface, self.rect, WHITE)
...
def icon_star(surface, rect, color):
...
def icon_speaker(surface, rect, color):
...
def icon_moon(surface, rect, color):
    cx, cy = rect.centerx, rect.centery - 10
    radius = rect.width * 0.22
    pygame.draw.circle(surface, color, (cx, cy), radius)
    pygame.draw.circle(surface, rect_bg_hint(surface, rect), (cx + int(radius * 0.5), cy - int(radius * 0.2)), radius)


def icon_sun(surface, rect, color):
    cx, cy = rect.centerx, rect.centery - 10
...
def icon_stop(surface, rect, color):
    cx, cy = rect.centerx, rect.centery - 10
    size = rect.width * 0.22
    pygame.draw.rect(surface, color, (cx - size / 2, cy - size / 2, size, size), border_radius=6)


def rect_bg_hint(surface, rect):
    # sample the button's own fill just outside the moon so the "bite" blends in
    return surface.get_at((rect.left + 2, rect.top + 2))[:3]
